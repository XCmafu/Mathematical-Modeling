%% 自适应权重的粒子群算法PSO: 求解函数y = x1^2+x2^2-x1*x2-10*x1-4*x2+60在[-15,15]内的最小值
% 求最大值，需要改权重w的条件 54-58行
clear; clc
...
x = zeros(n, narvs);
for i = 1: narvs
    x(:,i) = x_low(i) + (x_up(i)-x_low(i))*rand(n,1);    % 随机初始化粒子所在的位置在定义域内
end
...
for d = 1:K  % 开始迭代，一共迭代K次
    for i = 1:n   % 依次更新第i个粒子的速度与位置
        ...
        if  Obj_fun2(pbest(i,:)) < Obj_fun2(gbest)  % 如果第i个粒子的适应度小于所有的粒子迄今为止找到的最佳位置对应的适应度
            gbest = pbest(i,:);   % 那就更新所有粒子迄今为止找到的最佳位置
        end
    end
    fitbest(d) = Obj_fun2(gbest);  % 更新第d次迭代得到的最佳的适应度
    pause(0.1)  % 暂停0.1s

%% 自适应权重，带压缩因子，自动退出迭代循环的粒子群算法PSO: 求解四种不同测试函数的最小值
% 求最大值，权重w需要改一下
clear; clc
...
%         end
    ...
    delta_fit = abs(Obj_fun3(gbest) - Obj_fun3(temp));   % 相邻两次迭代适应度的变化量
    if delta_fit < tolerance  % 判断这个变化量和“函数变化量容忍度”的相对大小,变化极小
        Count = Count + 1;
    else
        Count = 0;  % 变化极大，计数器清0
    end   
